...
router.get("/history", authenticateToken, async (req, res) => {
  try {
    ...
    debug("Found upload sessions:", sessions.length);
...
router.post(
  ...
  async (req, res) => {
    try {
      debug("Restore request received for session:", req.params.sessionId);
      ...
      if (!session) {
        debug("Session not found:", req.params.sessionId);
        return res.status(404).json({ error: "Session not found" });
      }

      debug("Session found:", session._id);
      ...
      debug("Found travel data records:", travelData.length);

      if (!travelData.length) {
        return res
          .status(404)
          .json({ error: "No data found for this session" });
      }
      ...
      debug("Sending response with session ID:", response.session._id);
      res.json(response);
    } catch (error) {
